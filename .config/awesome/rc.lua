...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "c",      function () (mouse.current_client or client.focus):kill() end,
...
awful.rules.rules = {
    ...
    -- Floating clients.
    {
        rule_any = {
            instance = {
                "DTA", -- Firefox addon DownThemAll.d
    ...
    { rule_any = { class = { "AmneziaVPN" } }, properties = { floating = true } },
...
-- client.connect_signal("focus", function(c)
--     if c and c.pid then
--         -- Store the original nice value first
--         awful.spawn.easy_async("nice -n %i ps -o nice= -p "..c.pid, function(original_nice)
...
--                 text = "Process: "..(c.name or "unknown")..
--                        "\nPID: "..c.pid..
--                        "\nNice: "..(original_nice and original_nice:gsub("%s+", "") or "?").." → -9",
--                 timeout = 3,
...
--             text = "No valid PID for focused window",
--             timeout = 3,
--             screen = awful.screen.focused()
--         })
--     end
...
-- -- Optional: Notification when priority is reset
-- client.connect_signal("unfocus", function(c)
--     if c and c.pid then
--         awful.spawn("renice -n 0 -p " .. c.pid, false)
--         naughty.notify({
